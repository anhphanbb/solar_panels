...
def update_plot(time_step):
    ...
    if show_lines:
        for x in range(32, radiance_at_time.shape[1], 32):
            ax.axvline(x=x, color='green', linestyle='-')
        for y in range(64, radiance_at_time.shape[0], 64):
            ax.axhline(y=y, color='green', linestyle='-')
            
    # Display the box numbers with a semi-transparent background if enabled
    if show_numbers:
        for x in range(0, radiance_at_time.shape[1], 32):
            for y in range(0, radiance_at_time.shape[0], 64):
                box_label_x = x + 16
                box_label_y = y + 32
                ax.text(
                    box_label_x, box_label_y, f"({x//32},{y//64})",
