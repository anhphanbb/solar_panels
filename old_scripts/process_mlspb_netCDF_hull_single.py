# -*- coding: utf-8 -*-
"""
Save MLSP > threshold binary values of the two largest clusters into a new NetCDF file with expansion.
...
orbit_number = 1750  # Orbit number
...
dataset_filename, dataset_path = None, None
...
# # Expand cluster points, handling straight-line clusters
...
#         if hull.volume == 0:  # Degenerate case: cluster is a straight line
#             z_range = cluster_points[:, 0].min(), cluster_points[:, 0].max()
#             y_range = cluster_points[:, 1].min(), cluster_points[:, 1].max()
#             x_range = cluster_points[:, 2].min(), cluster_points[:, 2].max()

#             # Expand the bounding box
#             buffer = expansion_factor
#             expanded_z_range = (z_range[0] - buffer, z_range[1] + buffer)
#             expanded_y_range = (y_range[0] - buffer, y_range[1] + buffer)
#             expanded_x_range = (x_range[0] - buffer, x_range[1] + buffer)

#             # Select all points within the expanded bounding box
#             all_points = np.column_stack(np.where(mlsp > 0.1))
#             in_buffer = (
#                 (expanded_z_range[0] <= all_points[:, 0]) & (all_points[:, 0] <= expanded_z_range[1]) &
#                 (expanded_y_range[0] <= all_points[:, 1]) & (all_points[:, 1] <= expanded_y_range[1]) &
#                 (expanded_x_range[0] <= all_points[:, 2]) & (all_points[:, 2] <= expanded_x_range[1])
#             )
#             return np.unique(np.vstack((cluster_points, all_points[in_buffer])), axis=0)

#         else:  # Regular expansion for non-degenerate clusters
