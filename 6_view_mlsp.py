...
orbit_number = 1600  # orbit number
...
def update_plot(time_step):
    ...
    # Calculate the running average for mlsp over a window
    kernel = np.ones(running_average_window) / running_average_window
    averaged_mlsp = np.apply_along_axis(lambda m: np.convolve(m, kernel, mode='same'), axis=0, arr=mlsp)
    averaged_mlsp_at_time = averaged_mlsp[current_time_step, :, :]
    ...
    # Highlight boxes with averaged MLSP > threshold and display MLSP values
    solar_panel_count = 0
    ...
    if show_highlight:
        for box, coords in grid_boxes.items():
            ...
            # Use the averaged MLSP to determine highlights
            mlsp_value = averaged_mlsp_at_time[y_idx, x_idx]
            if mlsp_value > threshold:
                solar_panel_count += 1
                # Highlight the box
            ...
            # Display the MLSP value for each box
            box_center_x = (x_start + x_end) / 2
            box_center_y = (y_start + y_end) / 2
            ax.text(
                box_center_x, box_center_y, f"{mlsp_value:.2f}",
                color='red' if mlsp_value > threshold else 'black',
...
# Ensure slider and range slider callbacks are correctly connected
